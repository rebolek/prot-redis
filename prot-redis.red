...
parse-reply: func [
	...
	/binary	"Do not convert binary! to string!"
	/local
		crlf message length status error integer bulk multi
		msg ret out
] [
	crlf: #{0D0A}
	out: ret: make block! 1000
	message: [
...
send-redis: func [
	...
	client: open client
	b: make-bulk-request parse-dialect data
...
read-key: func [
	...
	/local member type post cmd ret
][
	member: none
	if all [path? key 2 = length? key][
		member: second key
...
comment [
	...
	actor: [
		...
		open: func [
			...
			redis-port/state/tcp-port: tcp-port: make none [
				scheme: 'tcp
